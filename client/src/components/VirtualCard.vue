  ...
  methods: {
    ...
    onShareCard(cardToken) {
      ...
      const recipientEmail = window.prompt(
        "What is your recipient's email? They'll be emailed a link to this card and prompted to create a password."
      );
      if (recipientEmail) {
        this.$http
          ...
          .then(body => {
            ...
            this.$emit("shared-card");
            this.$toast.success(
              `Your card ending in ${this.card.last_four} has been shared with ${recipientEmail}`
            );
          })
          .catch(() => {
